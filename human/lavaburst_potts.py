    ...
    for repl in repls:
        for chrom in chroms:
            ...
            for gamma in potts_gamma:
                print(f"\rmethod=lavaburst_potts line={line} repl={repl} chrom={chrom} gamma={gamma} in progress     ", end="")
                S = lavaburst.scoring.potts_score(matrix, gamma=gamma, binmask=good_bins)
                model = lavaburst.SegModel(S)
                segments = model.optimal_segmentation()
                np.savetxt(f"{out_location}lava_potts_{line}{repl}.20000.chr{chrom}.gamma{gamma}.txt", segments * res, delimiter="\t", fmt="%d")
